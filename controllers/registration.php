...
//Register Pay Online
if (isset($_POST['register_btn_online'])) {
    $id = $conn->real_escape_string($_POST['id']);
    $firstName = $conn->real_escape_string($_POST['firstName']);
    $lastName = $conn->real_escape_string($_POST['lastName']);
    $email = $conn->real_escape_string($_POST['email']);
    ...
    $query = "INSERT INTO applicants (firstName, lastName, email, phoneNum, lga, dateOfBirth, gender, height, occupation, igHandle, picture, regCode, biography)"
        . "VALUES ('$firstName', '$lastName', '$email', '$phoneNum', '$lga', '$dateOfBirth', '$gender', '$height', '$occupation', '$igHandle', '$picture_path', '$regCode', '$biography')";
